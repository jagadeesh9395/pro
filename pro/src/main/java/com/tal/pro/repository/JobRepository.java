...
public interface JobRepository extends MongoRepository<Job, String> {
    ...
    /**
     * Find all jobs posted by a specific recruiter that are not deleted
     * Uses $ne (not equal) true to handle both 'false' and null/missing values
     */
    @Query("{ 'postedBy.$id': ?0, 'deleted': { $ne: true } }")
    List<Job> findByPostedBy_IdAndDeletedFalse(ObjectId recruiterId);

    /**
     * Find all jobs posted by a specific recruiter (includes deleted)
     */
    List<Job> findByPostedBy_Id(String recruiterId);
     ...
     */
    long countByPostedBy_Id(String recruiterId);

    /**
     * Check if a job exists with the given ID and posted by the specified recruiter
     */
    boolean existsByIdAndPostedBy_Id(String jobId, String recruiterId);
